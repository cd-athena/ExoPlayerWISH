...
public class SampleQueue implements TrackOutput {
  ...
  /** Returns the current absolute start index. */
   public final int getFirstIndex() { return absoluteFirstIndex;}
  ...
  private synchronized int readSampleMetadata(
    ...
    if (!hasNextSample()) {
      Log.i("MINH", " Do not have next sample");
...
//        Log.i("access", "=== commitSample sourceIds: " + sourceIds[sourceIds.length-1]);
//      if (formats.length > 0)
//      Log.i("access", "=== commitSample formats: " + formats[formats.length-1].bitrate);
    }
  }
